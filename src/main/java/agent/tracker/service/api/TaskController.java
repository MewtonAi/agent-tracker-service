...
public class TaskController {
    ...
    public TaskController(TaskCommandService commandService, TaskQueryService queryService, TaskInputNormalizer inputNormalizer) {
    ...
    public TaskResponse create(
        ...
        Task task = commandService.createTask(new CreateTaskCommand(
            ...
            request.priority(),
            request.requestedBy(),
            inputNormalizer.requireIdempotencyKey(idempotencyKey)
        ));
    ...
    public ListTasksResponse list(
        ...
        TaskStatus filter = inputNormalizer.parseStatusFilter(status);
        String normalizedCursor = inputNormalizer.normalizeCursor(cursor);
        int normalizedLimit = inputNormalizer.normalizeLimit(limit);
        TaskListPage page = queryService.listTasks(filter, normalizedCursor, normalizedLimit);
    ...
    public TaskResponse updateStatus(
        ...
        Task task = commandService.updateTaskStatus(new UpdateTaskStatusCommand(
            taskId,
            request.status(),
            request.requestedBy(),
            inputNormalizer.requireIdempotencyKey(idempotencyKey)
